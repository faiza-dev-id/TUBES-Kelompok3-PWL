...
<aside class="w-64 min-h-screen bg-indigo-900 text-white flex flex-col">
    ...
    <nav class="flex-1 px-4 py-4 space-y-0.5">
        ...
        <a href="{{ route('kaprodi.mahasiswa.index') }}"
           class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm
                  {{ request()->routeIs('kaprodi.mahasiswa.index') || request()->routeIs('kaprodi.mahasiswa.show') ? 'bg-indigo-700 text-white' : 'text-indigo-200 hover:bg-indigo-700 hover:text-white' }}">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
