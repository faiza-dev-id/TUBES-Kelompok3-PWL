...
<main style="padding:32px;min-height:100vh;">
        <div class="mb-6">
            <a href="{{ route('admin.users.index') }}" class="text-sm text-blue-600 hover:underline flex items-center gap-1 mb-3">
                ← Kembali ke daftar user
            </a>
            <h1 class="text-2xl font-bold text-gray-800">Tambah User Baru</h1>
            <p class="text-sm text-gray-500 mt-1">Password default: <strong>password123</strong> — user wajib ganti sendiri setelah login pertama.</p>
        </div>
            ...
            <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="role" required
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        @foreach(['mahasiswa','admin','kaprodi','sekprodi'] as $r)
                    ...
                    <p class="text-xs text-gray-400 mt-1">
                        Catatan: untuk role <strong>mitra</strong>, gunakan menu Mitra agar data perusahaan juga tersimpan.
                    </p>
                </div>

                {{-- Informasi: password diatur oleh user sendiri --}}
                <div class="bg-amber-50 border border-amber-200 rounded-lg p-3">
                    <div class="flex items-start gap-2">
                        ...
                        <p class="text-xs text-amber-700">
                            Password default adalah <strong>password123</strong>. User harus mengubah password sendiri melalui menu Profil setelah login pertama kali.
                        </p>
                    </div>
                </div>
